...
anybasis_params['tc1'] = radvel.Parameter(value=2454528.63)#6340.313479)
anybasis_params['e1'] = radvel.Parameter(value=0.0142831)#0.003400)
anybasis_params['w1'] = radvel.Parameter(value=2.67222)#1.710423)
anybasis_params['k1'] = radvel.Parameter(value=70.9977)#71.400000)
anybasis_params['per2'] = radvel.Parameter(value=5208.57)#4825.000000)
anybasis_params['tc2'] = radvel.Parameter(value=2455958.43)#6340.313479)
anybasis_params['e2'] = radvel.Parameter(value=0.0926613)#0.019000)
anybasis_params['w2'] = radvel.Parameter(value=0.41539)#0.767945)
anybasis_params['k2'] = radvel.Parameter(value=52.5992)#48.290000)
anybasis_params['per3'] = radvel.Parameter(value=44.3845)#44.417500)
anybasis_params['tc3'] = radvel.Parameter(value=2456340.313479)
anybasis_params['e3'] = radvel.Parameter(value=0.020000)
anybasis_params['w3'] = radvel.Parameter(value=0.890118)
anybasis_params['k3'] = radvel.Parameter(value=10.180000)
anybasis_params['per4'] = radvel.Parameter(value=261.246)
anybasis_params['tc4'] = radvel.Parameter(value=2455238.53)
anybasis_params['e4'] = radvel.Parameter(value=0.139143)
anybasis_params['w4'] = radvel.Parameter(value=-2.48775)
anybasis_params['k4'] = radvel.Parameter(value=4.72425)

time_base = 2456340.313479
#data = pd.read_csv('/data/radvel/input_dir/75732/data/dynamic_custom/remove_outliers.csv')
data = pd.read_csv('~/Academics/Astronomy/Planets/rvdata/vst75732.csv')
bin_t, bin_vel, bin_err, bin_tel = radvel.utils.bintels(data['time'].values, data['mnvel'].values,
